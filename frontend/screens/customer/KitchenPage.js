...
const KitchenPageScreen = ({navigation}) => {
  ...
  for (const item of temp_items) {
    initial_item_counts[item.id] = {count: 0};
  }
  const [itemCounts, setItemCounts] = useState(initial_item_counts)
  const [showModal, setShowModal] = useState(false);
  const [modalState, setModalState] = useState({itemName: "", price: "",description: "",imgLink: ""})
  ...
  return (
        ...
        alignItems: 'center'
          ...
          marginHorizontal: 30
            ...
            <View style={{width: 80, height: 80, borderRadius: 10, alignContent:"center"}}>
              <Image style={{width: 80,height:80, borderRadius: 10}} source={{uri:modalState.imgLink}} />
            </View>
            <View style={{
            flexDirection:'column',
            justifyContent:'space-between',
            ...
            <Text style={{textAlign:"left",fontWeight: 'bold', fontSize: 18}}>{modalState.itemName}</Text>
            <BlankDivider height={8}/>
    ...
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      {
        temp_items.map(item =>
          <TouchableOpacity key={item.id} onLongPress={() => {setModalState({itemName: item.name,price:item.price,description: item.description,imgLink: item.img}); setShowModal(true)}}> 
            <OrderMenuItem
              itemName={item.name}
              price={item.price}
              description={item.description}
              count={itemCounts[item.id].count}
              setCount={diff => {setItemCounts({...itemCounts, [item.id]: {count: Math.max(itemCounts[item.id].count+diff,0)}})}}
              imgLink={item.img}
            />
          </TouchableOpacity>
        )
